...
def para_ul_random_walk(city, model_name, ulist, ul_graph, lu_graph, walk_len, walk_times):
    ...
    print "Total Number of Cores:", core_num
    job_num = core_num / 2
    print "Number of jobs:", job_num
    # do not use shared memory
    Parallel(n_jobs=job_num)(delayed(ul_random_walk_core)
                             (city, model_name, u, ul_graph, lu_graph, walk_len, walk_times)
                             for u in ulist)
...
def ul_random_walk_core(city, model_name, start_u, ul_graph, lu_graph, walk_len, walk_times):
    ...
    for i in range(walk_times):
        ...
        pd.DataFrame(temp_walk).to_csv('dataset/'+city+'/emb/' + city+'_'+model_name+'.walk',
